	...
	<input type="hidden" name="id" value="${memID}">
		<div class="container my-container">
			<div class="row"> <!-- 12개의 칼럼 -->
				...
				<div class="col-lg-8">
					<h2 class="form-control" align="center">게시판</h2>
							...
							<tr style="text-align: center">
								...
								<td>
									<c:if test="${memID.equals(list.getMemId())}">
										<a class="btn btn-success float-right" href="BoardServlet?cmd=update&bdId=${list.getBdId()}">글 수정</a>
									</c:if>
								</td>
							</tr>
						</c:forEach>
					...
					<c:if test="${prev == true}">
						<a class="btn btn-success"href="BoardServlet?cmd=boardList&pageNum=${pageNum-1}" style = "margin:2px">이전</a>
					</c:if>
					<c:if test="${next == true}">
						<a class="btn btn-success" href="BoardServlet?cmd=boardList&pageNum=${pageNum+1}" style = "margin:2px">다음</a>
					</c:if>
